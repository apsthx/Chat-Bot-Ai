            ...
            <div class="card-body">
                <h4 class="card-title">
                    ...
                    <span style="float: right">
                        ...
                        <a href="<?php echo base_url('groupmenu/sortmenu/' . $group_menu_id); ?>" class="btn btn-sm btn-rounded btn-outline-info"><i class="fa fa-sort"></i> จัดเรียงเมนู</a>
                        <a href="<?php echo base_url('groupmenu'); ?>" class="btn btn-sm btn-rounded btn-outline-inverse"><i class="fa fa-close"></i> ยกเลิก</a>
                    </span>
                </h4>
...
<div id="add-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="add-form" method="post" action="<?php echo base_url('groupmenu/addmenu'); ?>" autocomplete="off">
...
<div id="edit-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="edit-form" method="post" action="<?php echo base_url('groupmenu/editmenu'); ?>" autocomplete="off">
...
<script>
    ...
    function modaledit(menu_id) {
        ...
        $('#menu_name').val('');
        $('#menu_link').val('');
        let url = service_base_url + 'groupmenu/getmenu';
        $.ajax({
            url: url,
            method: 'POST',
            dataType: 'JSON',
    ...
    function modaldelete(group_menu_id, menu_id) {
        let url = service_base_url + 'groupmenu/deletemenu/' + group_menu_id + '/' + menu_id;
